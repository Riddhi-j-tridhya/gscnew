    ...
    ?><ul class="news-nav"><?php
		...
		function ss_search_filter( $query ) {
			if ( !$query->is_admin && $query->is_search && $query->is_main_query() ) {
				$query->set( 'post__not_in', array( 89770 ) );
			}
		}
        ...
        function gsc_financial_statistics_table() {
            ...
            $PerformanceArr = array(
             ...
             $labels = array(
        'share_capital' => get_field("share_capital_title", $post->ID),
        'reserves_and_other_funds' => get_field("reserves_and_other_funds_title", $post->ID),
        'total_advances' => get_field("total_advances_title", $post->ID),
        'deposits' => get_field("deposits_title", $post->ID),
        'profit__loss' => get_field("profit_loss_title", $post->ID),
        'dividend' => get_field("dividend_title", $post->ID),
    );
            ob_start();
...
add_shortcode("gsc_financial_charts", "gsc_financial_chart");

        function gsc_financial_chart() {
          ?>
...
function mynewlearnshotcode() {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        $ssearch = $_GET['ss'];
        global $wpdb;
        $result = $wpdb->get_results("SELECT * FROM cust_data  where (`account_name`  LIKE '%" . $search . "%'  AND `address_one`  LIKE '%" . $ssearch . "%' ) OR (`account_name`  LIKE '%" . $search . "%' 
		                              AND  `address_four`  LIKE '%" . $ssearch . "%' ) OR (`account_name`  LIKE '%" . $search . "%' 
		                              AND  `address_two`  LIKE '%" . $ssearch . "%' ) OR (`account_name`  LIKE '%" . $search . "%' 
		                              AND  `address_three`  LIKE '%" . $ssearch . "%' ) ");

        echo '<table class="table table-bordered "><thead>';
        echo '<tr><th class="">Name</th><th class="">Address</th>';
        foreach ($result as $print) {
            echo "<tr ><td>$print->account_name</td><td>$print->address_one ", " $print->address_two ", " $print->address_three ", " $print->address_four</td></tr>";
        }
        echo '</thead>';
        echo '</table>';
        if (empty($result)) {
...
function enqueue_custom_styles() {
    ...
    // Enqueue your custom styles with a higher priority
    wp_enqueue_style('custom-styles', get_template_directory_uri() . 'http://beta.gscbank.co.in/wp-content/plugins/js_composer/assets/css/js_composer.min.css?ver=6.10.0', array('js_composer_front'), '1.0', 'all');
}
add_action('wp_enqueue_scripts', 'enqueue_custom_styles', 999);
...
/*banner class*/
function add_common_class_to_wpbakery_elements($output, $post_id) {
    if (is_page()) { // Condition to target specific pages, you can modify this if needed
        $common_class = 'main-banner'; // Define the common class you want to add
        $output = str_replace('vc_column wpb_column', 'vc_column wpb_column ' . $common_class, $output);
    }
    return $output;
}
add_filter('vc_shortcode_output', 'add_common_class_to_wpbakery_elements', 10, 2);


/*shortcode annual report*/

// Shortcode callback function
// Add shortcode
add_shortcode("demat_account_investor_deta", "demat_account_investor_deta");

// Shortcode callback function
function demat_account_investor_deta() {
    ...
    <form name="blah_blah">
        ...
        $demat_account_data = get_field('demat_account_year_month_dropdown');
        ?>

        <!-- Year Dropdown -->
        <label for="year">Select Year : </label>
        <select id="year" name="year" class="demat-acc-drop custom-search-one wpcf7-form-control wpcf7-select wpcf7-validates-as-required" required onchange="updateMonthDropdown()">
            <option value="">--Select--</option>

            <?php
            // Array to store unique years
            $unique_years = array();

            foreach ($demat_account_data as $data) :
                $year = $data['year'];
                $month = $data['month'];
                ...
                if (!in_array($year, $unique_years)) {
                    // If not, add it to the array and display in the dropdown
                    $unique_years[] = $year;
                    ?>
        ...
        <select id="month" name="month" class="custom-search-one wpcf7-form-control wpcf7-select wpcf7-validates-as-required" required>
            <option value="">--Select--</option>
            <?php foreach ($demat_account_data as $data) :
                $year = $data['year'];
                $month = $data['month'];
                $pdf_link = $data['pdf_link'];
                ?>
                <option value="<?php echo $month; ?>" data-year="<?php echo $year; ?>"><?php echo $month; ?></option>
            <?php endforeach; ?>
        </select>

        <!-- View Button -->
        <input type="button" class="btn btn-primary btn-sm" name="Submit" value="View" onClick="openPdf()">
    ...
    function updateMonthDropdown() {
        ...
        var dematAccountData = <?php echo json_encode($demat_account_data); ?>;

        // Reset month dropdown
        monthDropdown.innerHTML = '<option value="">--Select--</option>';

        // Populate month dropdown with months corresponding to the selected year
        dematAccountData.forEach(function(data) {
            var year = data['year'];
    ...
    function openPdf() {
        ...
        if (selectedYear && selectedMonth) {
            ...
            dematAccountData.forEach(function(data) {
                ...
                if (dataYear === selectedYear && dataMonth === selectedMonth) {
                    pdfLink = data['pdf_link']['url'];
                }
            });
            ...
            if (pdfLink) {
                ...
                alert('PDF link not found for the selected year and month.');
            }
        } else {
