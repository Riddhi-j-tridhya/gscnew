...
if ( ! function_exists( 'visualcomposerstarter_entry_date' ) ) :
	...
	function visualcomposerstarter_entry_date() {
		...
		/* translators: %s: post date */
			esc_html__( '%1$sPosted on%2$s %3$s', 'gsc-bank' ),
	...
	function visualcomposerstarter_entry_meta() {
		...
		<ul class="entry-meta">
			...
			<?php if ( get_the_category_list() ) : ?>
				<li class="entry-meta-category"><?php the_category( _x( ', ', 'Used between list items, there is a space after the comma.', 'gsc-bank' ) ); ?></li>
			<?php endif; ?>
			<?php if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) : ?>
				<li class="entry-meta-comments">
					<?php comments_popup_link( esc_html__( 'Leave a comment', 'gsc-bank' ), esc_html__( '1 Comment', 'gsc-bank' ), esc_html__( '% Comments', 'gsc-bank' ) ); ?>
				</li>
			<?php endif; ?>
		</ul>
		<?php
		...
		if ( in_array( get_post_type(), array(
			...
			<div class="entry-meta">
				<?php if ( in_array( get_post_type(), array(
					...
					<?php echo esc_html_x( 'On', 'Post meta', 'gsc-bank' ); ?>
					<?php if ( in_array( get_post_type(), array(
						'post',
						'attachment',
					), true ) ) : ?>
					...
					<?php endif; ?>
					<?php if ( 'post' === get_post_type() ) : ?>
						<?php echo esc_html_x( 'by', 'Post meta', 'gsc-bank' ); ?>
				...
				<?php if ( ! empty( $categories ) ) : ?>
					<?php echo esc_html_x( 'to', 'Post meta', 'gsc-bank' ); ?>
					<?php the_category( _x( ', ', 'Used between list items, there is a space after the comma.', 'gsc-bank' ) ); ?>
				<?php endif; ?>
			</div>
			<?php
		}
...
if ( ! function_exists( 'visualcomposerstarter_entry_tags' ) ) :
	...
	function visualcomposerstarter_entry_tags() {
		the_tags( '<div class="entry-tags"><span class="screen-reader-text">' . esc_html( _x( 'Tags', 'Used before tag names.', 'gsc-bank' ) ) . '</span>','', '</div>' );
	}
endif;
		...
		<div class="comment-wrapper">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					...
					printf( esc_html__( '%1$s %2$s says: %3$s', 'gsc-bank' ), '<cite>' . get_comment_author_link() . '</cite>', '<span class="says">', '</span>' ); ?>
				</div>
				<div class="comment-metadata">
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
						<?php
						/* translators: 1: date, 2: time */
						printf( esc_html__( 'On %1$s at %2$s','gsc-bank' ), get_comment_date(),  get_comment_time() ); ?>
					</a>
					<?php edit_comment_link( esc_html__( '(Edit)', 'gsc-bank' ), '  ', '' ); ?>
					<?php if ( '0' === $comment->comment_approved ) : ?>
						<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'gsc-bank' ); ?></em>
					<?php endif; ?>
				</div>
			</footer>
