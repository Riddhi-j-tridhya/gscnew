...
class CampaignsList extends \WP_List_Table
{
	...
	{
		$meta = get_post_meta($item['ID'], 'fpframework_meta_settings', true);
		echo esc_html($this->getMode($meta));
	}

	public function column_trigger_point($item)
	{
		$meta = get_post_meta($item['ID'], 'fpframework_meta_settings', true);
	...
	public function column_last_viewed($item)
	{
		$last_date_viewed = \FireBox\Core\Helpers\BoxHelper::getCampaignLastDateViewed($item['ID']);
		
		echo '<span title="' . esc_attr(firebox()->_('FB_LAST_DAY_OPENED_DESC')) . '">' . esc_html($last_date_viewed ? $last_date_viewed : fpframework()->_('FPF_N/A')) . '</span>';
	}
	...
	protected function handle_row_actions($item, $column_name, $primary)
	{
		...
		$actions['duplicate'] = '<a href="admin.php?action=fb_duplicate_post_as_draft&post=' . $post['ID'] . '" title="' . firebox()->_('FB_DUPLICATE_CAMPAIGN') . '" rel="permalink">' . fpframework()->_('FPF_DUPLICATE') . '</a>';
		...
		{
			$actions['clear_cookie'] = '<a class="firebox_red_text_color" href="admin.php?action=fb_clear_cookie&post=' . $post['ID'] . '" title="' . firebox()->_('FB_CLEAR_COOKIE') . '" rel="permalink">' . firebox()->_('FB_HIDDEN_BY_COOKIE') . '</a>';
		}
	...
	protected function get_bulk_actions()
	{
		return [
			...
			'reset_stats'  => __( 'Reset Impressions', 'firebox' ),
		];
	}
	...
	{
		return '<div>Last Modified</div>' . get_date_from_gmt($item['date']);
	}

	private function getCampaignStatus()
	{
		return isset($_GET['status']) ? sanitize_key($_GET['status']) : '';
	}
	...
	function prepare_items()
	{
		...
		else
		{
			if ( isset( $_REQUEST['post_status'] ) && in_array( $_REQUEST['post_status'], $avail_post_stati, true ) ) {
				$total_items = $this->total_campaigns_data[ $_REQUEST['post_status'] ];
	...
	private function getMode($meta)
	{
		...
		$mode = $mode === 'popup' ? 'classic_popup' : $mode;

		return firebox()->_('FB_' . strtoupper($mode));
	}
