...
abstract class Metric
{
	...
	protected function applyFilters()
	{
		...
		{
			$this->sql_filters .= 'AND ' . $table_name . '.box IN (%s)';
			$this->query_placeholders[] = implode(',', array_map('intval', $this->filters['campaign']['value']));
		}

		// Country
		if (array_key_exists('country', $this->filters) && isset($this->filters['country']['value']) && is_array($this->filters['country']['value']))
		{
			$this->sql_filters .= 'AND ' . $table_name . '.country IN (' . implode(',', array_fill(0, count($this->filters['country']['value']), '%s')) . ')';
			$this->query_placeholders = array_merge($this->query_placeholders, $this->filters['country']['value']);
		}

		// Device
		if (array_key_exists('device', $this->filters) && isset($this->filters['device']['value']) && is_array($this->filters['device']['value']))
		{
			$this->sql_filters .= 'AND ' . $table_name . '.device IN (' . implode(',', array_fill(0, count($this->filters['device']['value']), '%s')) . ')';
			$this->query_placeholders = array_merge($this->query_placeholders, $this->filters['device']['value']);
		}

		// Event
		if (array_key_exists('event', $this->filters) && isset($this->filters['event']['value']) && is_array($this->filters['event']['value']))
		{
			$this->sql_filters .= 'AND ld.event IN (' . implode(',', array_fill(0, count($this->filters['event']['value']), '%s')) . ')';
			$this->query_placeholders = array_merge($this->query_placeholders, $this->filters['event']['value']);
		}
	...
	protected function getJOINs()
	{
		...
		// Skip
		if (get_class($this) === 'FireBox\Core\Analytics\Metrics\AverageTimeOpen')
		{
			return $joins;
		}
