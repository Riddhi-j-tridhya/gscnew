...
class Submissions extends \WP_List_Table
{
	...
	public function __construct($args = [])
	{
		...
		$this->selected_form_id = isset($_GET['form_id']) ? sanitize_key($_GET['form_id']) : (isset($this->forms[0]['id']) ? $this->forms[0]['id'] : null);
	...
	{
		$url = admin_url('admin.php?page=firebox-submissions&task=edit&id=' . $item->id);
	...
	public function process_bulk_action()
	{
		...
		else if ($action === 'delete')
		{
			...
			$wpdb->query("DELETE FROM $s_table_name WHERE id IN($ids)");
			...
			$wpdb->query("DELETE FROM $sm_table_name WHERE submission_id IN($ids)");
	...
	function prepare_items()
	{
		...
		$offset = isset($_GET['paged']) ? max(0, intval($_GET['paged'] - 1) * $this->per_page) : 0;

		$orderby = isset($_GET['orderby']) && in_array($_GET['orderby'], $this->getAllowedOrders()) ? sanitize_key($_GET['orderby']) : 'created_at';
		$order = isset($_GET['order']) && in_array($_GET['order'], ['asc', 'desc']) ? sanitize_key($_GET['order']) : 'desc';
		...
		$status = isset($_GET['status']) ? sanitize_key($_GET['status']) : false;
		if ($status)
		{
